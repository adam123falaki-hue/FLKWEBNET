...
export default function Home() {
  ...
  const whatsappNumber = "212600000000"; // بدل هاد الرقم برقمك
  ...
  const handleNavClick = (e: React.MouseEvent<HTMLAnchorElement>, href: string) => {
    if (href.startsWith('#')) {
      e.preventDefault();
      setMobileMenuOpen(false); // سد القائمة ف الموبايل
  ...
  return (
    <div className="bg-[#080d1a] text-slate-100 min-h-screen font-sans selection:bg-[#ef4444] selection:text-white relative overflow-x-hidden scroll-smooth" dir="ltr">
      ...
      >
        {/* LOGO */}
        <Link href="/" className="flex items-center shrink-0 group py-1 z-50">
          <img 
            src="/logo121.png" 
            alt="FLK WEB NET" 
        ...
        <div className="flex items-center gap-3 z-50">
          ...
          <button 
            ...
            className="md:hidden text-slate-200 focus:outline-none p-2.5 bg-slate-900/90 border border-slate-700/80 rounded-xl hover:bg-slate-800 transition active:scale-95"
      ...
      {mobileMenuOpen && (
        <div className="md:hidden fixed inset-0 top-20 bg-[#080d1a]/98 backdrop-blur-2xl z-40 px-6 py-8 flex flex-col justify-between border-b border-slate-800 shadow-2xl animate-in fade-in slide-in-from-top-4 duration-200">
      ...
      {/* HERO SECTION */}
      <main className="relative z-10 flex flex-col items-center justify-center pt-36 pb-20 px-6 md:px-12 text-center max-w-6xl mx-auto">
      ...
      {/* FOOTER */}
      <footer className="border-t border-slate-800/80 bg-[#050811] py-12 px-6 md:px-16 text-center md:text-left">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
          <div>
            <Link href="/" className="flex items-center justify-center md:justify-start mb-2 group">
              <img 
                src="/logo121.png" 
                alt="FLK WEB NET" 
            ...
            <p className="text-slate-500 text-sm">© {new Date().getFullYear()} FLK WEB NET. Tous droits réservés.</p>
          </div>

          <div className="flex gap-8 text-sm font-semibold text-slate-400">
            <a href="#services" onClick={(e) => handleNavClick(e, '#services')} className="hover:text-[#22d3ee]">Services</a>
            <a href="#projects" onClick={(e) => handleNavClick(e, '#projects')} className="hover:text-[#22d3ee]">Projets</a>
            <button onClick={() => setIsEstimatorOpen(true)} className="hover:text-[#ef4444] transition">Estimateur</button>
          </div>
        </div>
      </footer>
